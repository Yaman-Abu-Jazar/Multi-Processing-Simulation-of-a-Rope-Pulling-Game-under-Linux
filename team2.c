...
int main(int argc, char **argv){
    ...
    time2 = time_to_rejoin;

    id = atoi(argv[1]);

    pipes_2[0] = atoi(argv[2]);
    pipes_2[1] = atoi(argv[3]);
    ...
    if(write(pipes_2[1], &info, sizeof(PlayerInfo)) == -1){
    ...
    if(read(pipes_2[0], &info, sizeof(PlayerInfo)) == -1){
...
void user_handler1(int sig){
    printf("player %d in team 2 is ready in his position %d\n", info.player_id, info.position);
    sleep(2);
}
...
void user_handler2(int sig){
    ...
    if(falling_flag == 1){
        ...
        if(time2 == 0){
            ...
            time2 = time_to_rejoin;
            chance = 5;
            printf("Player %d in team 2 has came back.\n", info.player_id);
        }
    }
    // Generate a random number between 0 and 99
    if ((rand() % 100) < chance) {
        printf("Player %d in team 2 has fallen PID = %d  He will come back after %d seconds.\n",info.player_id, info.pid, time_to_rejoin);
        chance = 0;
        falling_flag = 1;
        multiplied_energy = 0;
    ...
    if(write(pipes_2[1], &multiplied_energy, sizeof(int)) == -1){
        perror("Write Pipe Error.\n");
        exit(-7);
    }
    sleep(2);
